...
gradlePlugin {
    plugins {
        create("ServiceApiArchPlugin") {
            id = "laplacian-arch.model.service-api-arch"
            implementationClass = "laplacian_arch.service_api_arch.gradle.ServiceApiArchPlugin"
        }
    }
}
...
publishing {
    ...
    publications {
        create<MavenPublication>("mavenJava") {
            artifactId = "laplacian-arch.model.service-api-arch"
            artifact(sourcesJar)
        }
        create<MavenPublication>("model") {
            artifactId = "laplacian-arch.model-def.service-api-arch"
            artifact(modelJar)
        }
    }
}